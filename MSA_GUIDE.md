...
| Phase 9 | **account-service batch 엔드포인트 + AccountInternalController 조회 API service 이관** | ✅ 완료 (2026-04-24) |
| Phase 9 | **study-service AccountFeignClient — 스터디 페이지 멤버 batch 조회 (백엔드)** | ✅ 완료 (2026-04-24) |
| Phase 9 | **frontend-service 멤버 카드 HTML 렌더링 (defensive UI 패턴 — 코드 변경 0줄)** | ✅ 완료 (2026-04-24) |
| Phase 8/9 | 묶음 통합 테스트 (이메일 인증 접근 제한 + 멤버 카드 렌더링) | 🔲 대기 중 |

**Phase 9 코드 작업 전부 완료 — 묶음 통합 테스트가 다음 작업**

Phase 9 의 세 단계가 모두 끝났다. 흥미로운 점은 3단계(frontend HTML 렌더링) 가
실제로는 **코드 변경이 한 줄도 필요 없는** 상태였다는 것이다. 양균님이 frontend-service 의
`MemberDto` 와 `study/view.html`, `study/members.html` 을 처음 작성하실 때 이미
"언젠가 batch API 가 생기면 여기에 닉네임이 채워질 것" 이라는 미래를 가정하고
fallback (Jdenticon 자동 아바타, 엘비스 연산자, # 링크 처리) 을 미리 깔아두셨기 때문이다.
이것이 잘 설계된 데이터 계약(data contract) + defensive UI 패턴의 위력이다.

다음 작업은 **Phase 8 세 번째 기능과 Phase 9 의 검증을 묶어서 진행하는 통합 테스트** 다.
양균님 결정으로 두 기능의 검증을 한 세션에 묶는다 — 컨텍스트 전환 비용을 줄이고,
여러 기능이 함께 동작할 때 드러나는 통합 이슈를 함께 잡기 위한 의도적 선택.
...
**Phase 9 신규 의존 관계 (서비스 간 호출):**
- study-service → account-service: `AccountFeignClient.getAccountsBatch()`
  스터디 페이지 멤버 카드에 nickname/profileImage 를 채우기 위해 batch 조회.
  account-service 장애 시 graceful degradation (빈 map 반환 → 화면은 정상, 닉네임만 빈 상태).
...
## 스터디 페이지 멤버 카드 데이터 흐름 (Phase 9 신규)
...
[frontend-service:8090]
  StudyPageController
       ↓
  StudyInternalClient.getStudyPageData(path, accountId)
       ↓
[study-service:8083]
  StudyInternalController.getStudyPageData()
    1. studyRepository.findByPath(path) — DB 에서 스터디 + managerIds + memberIds 로드
    2. 권한 플래그(isManager, isMember, hasPendingRequest) 계산
    3. fetchAccountMap(allMemberIds) ← managerIds + memberIds 합집합
       ↓
       AccountFeignClient.getAccountsBatch()
       ↓
[account-service:8081]
  POST /internal/accounts/batch  { ids: [1, 5, 12, ...] }
  AccountInternalService.getAccountsBatch()
    → SELECT * FROM account WHERE id IN (...)
    → Map<Long, AccountSummaryResponse> 반환
       ↑
[study-service]
  4. toMemberInfo(id, accountMap) 로 MemberInfo 리스트 조립
     (map 에서 id 로 O(1) 조회 → nickname/profileImage/bio 채움)
  5. StudyPageDataResponse 반환
       ↑
[frontend-service]
  Thymeleaf view.html 또는 members.html 렌더링
    ${manager.nickname ?: ''}            — 닉네임 (null 안전)
    ${#strings.isEmpty(manager.profileImage)} 분기
      → SVG Jdenticon (id 기반 자동 아바타)
      → IMG 실제 프로필 사진
       ↑
[브라우저]
  화면에 멤버 카드 표시
...
**Graceful Degradation (account-service 장애 시)**:
- study-service 의 `fetchAccountMap` 이 `FeignException` 을 catch → `Collections.emptyMap()` 반환
- frontend-service 는 nickname=null, profileImage=null 인 MemberInfo 를 받음
- Thymeleaf 의 fallback 분기가 작동 → 카드는 그려지되 닉네임 자리는 비고 Jdenticon 만 표시
- 스터디 페이지의 핵심 기능(스터디 정보 표시) 은 정상 동작
...
## study-service 구조 (Phase 4 + Phase 5 + Phase 8 + Phase 9 누적)
...
│     AccountFeignClient.java         ← Phase 9 신규 — account-service batch 호출
├── client/dto/
│     EventSummaryDto.java
│     AccountSummaryDto.java          ← Phase 9 신규 (id/nickname/profileImage/bio 4필드, anti-corruption)
│     AccountBatchRequest.java        ← Phase 9 신규 (record + Set<Long> 팩토리)
├── common/                            ← Phase 8 세 번째 기능 신규
│     EmailVerifiedGuard.java          EmailVerifiedGuard.require(emailVerified)
├── controller/
...
│                                      getStudyPageData() 가 AccountFeignClient 로 멤버 batch 조회 ← Phase 9
│                                      fetchAccountMap private 헬퍼 — 빈 가드 + Feign 호출 + FeignException fallback
│                                      (graceful degradation: account-service 장애 시에도 페이지 정상 렌더링)
├── dto/response/
...
## account-service 구조 (Phase 9 후 최신)
...
│     AccountInternalController.java                       ← Phase 9 에서 모든 핸들러를 service 위임으로 정리
...
│         findAllByIdIn(Collection<Long>)                ← Phase 9 신규 (Spring Data JPA IN 절 쿼리)
├── security/
...
│     SignUpService.java
│     AccountInternalService.java                         ← Phase 8 첫 번째 기능 신규 + Phase 9 대폭 확장
│         updateRole(targetId, requesterId, newRole) — 자기 자신 검증 / role 화이트리스트 / DB UPDATE
│         getAccountSummary / getAccountFull / getAccountTags / getAccountZones    ← Phase 9 (controller 에서 이관)
│         getAccountByNickname (Optional 반환 — 404 의미 보존)                       ← Phase 9 (controller 에서 이관)
│         listAccounts (관리자용 페이지네이션)                                       ← Phase 9 (controller 에서 이관)
│         getAccountsBatch(List<Long> ids) — Map<Long, AccountSummaryResponse>     ← Phase 9 신규
│         findAccountOrThrow private 헬퍼로 "계정을 찾을 수 없습니다: id=..." 메시지 일관화
└── ...
```

---

## frontend-service 구조 (Phase 9 후 최신)
...
│     │            (MemberDto: id/nickname/profileImage/bio — Phase 9 batch 응답을 받는 그릇)
│     ├── event/
...
[Phase 9 — 멤버 카드 렌더링은 코드 변경 없이 작동]
study/view.html, study/members.html 둘 다 처음 작성 시점부터 nickname/profileImage/bio
fallback 분기를 미리 깔아두었기 때문에, study-service 가 채워서 보내는 데이터가 곧바로
화면에 표시된다. 이것이 잘 설계된 데이터 계약(data contract) + defensive UI 패턴의 위력.
```
...
| 018-Phase5 | 모임 수정 시 eventType(선착순/관리자확인) 변경 불가 | UpdateEventRequest DTO 와 Event.update() 양쪽에 eventType 누락 / Jackson 의 조용한 무시 |
| 019-Phase9 | Java 람다식에서 "변수는 final 또는 유사 final이어야 합니다" | StudyInternalController.getStudyPageData 의 if/try/catch 다중 할당 변수를 람다가 캡처 → fetchAccountMap 메서드 추출로 해결 |

*Phase 5 관련 상세 트러블슈팅은 `TroubleShooting/TroubleShooting_016.MD`, `TroubleShooting_017.MD` 참고.*
*Phase 9 람다 캡처 에러의 자세한 설명은 `TroubleShooting/TroubleShooting_019.MD` 참고.*

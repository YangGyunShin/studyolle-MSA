...
14. [요청에서 응답까지 — 전체 흐름 완전 해부](#14-요청에서-응답까지--전체-흐름-완전-해부)
...
## 14. 요청에서 응답까지 — 전체 흐름 완전 해부

이 섹션은 "사용자가 회원가입 버튼을 누르는 순간부터 화면에 결과가 표시되기까지" 내부에서 실제로 어떤 일이 일어나는지를 TCP 연결 수준부터 코드 수준까지 낱낱이 설명한다. 앞선 섹션들에서 다뤘던 내용들이 여기서 하나로 연결된다.

---

### 전제: JavaScript는 브라우저에서 실행된다

이 섹션 전체를 이해하기 위해 반드시 먼저 이해해야 할 것이 하나 있다. Thymeleaf 코드는 **서버**에서 실행되지만, JavaScript 코드는 **브라우저**에서 실행된다.

모노리틱에서 `th:text="${nickname}"`은 서버 컴퓨터 안에서 실행됐다. 서버가 DB에서 닉네임을 가져와서 HTML 안에 집어넣고, 완성된 HTML을 브라우저에 내려보냈다.

JavaScript의 `fetch()`는 다르다. 서버가 내려보낸 HTML 안에 JavaScript 코드가 들어있고, 브라우저가 그 HTML을 받아서 파싱하면서 JavaScript를 자신의 메모리에 올려 **브라우저가 직접 실행**한다. `fetch()`가 HTTP 요청을 보내는 주체는 서버가 아니라 **사용자의 컴퓨터에 있는 브라우저**다.

이 전제를 이해하면, "왜 두 번째 요청에서 frontend-service가 관여하지 않는가"라는 의문이 자연스럽게 풀린다.

---

### HTTP 요청의 본질: URL은 목적지 주소다

`fetch("http://localhost:8080/api/auth/signup")`에서 `localhost:8080`은 단순한 목적지 주소다. 브라우저는 이 URL을 보고 "내 컴퓨터(`localhost`)의 `8080`번 포트로 TCP 연결을 열겠다"고 결정한다. 거기에 api-gateway가 대기하고 있기 때문에 api-gateway로 요청이 가는 것이다.

만약 `8081`이었다면 account-service로 직접 갔을 것이고, `8090`이었다면 frontend-service로 갔을 것이다. 어느 서버로 요청이 가는지는 오직 URL 안의 포트 번호로만 결정된다. frontend-service가 중간에서 중계해주는 것이 전혀 아니다.

```javascript
// 이 한 줄이 어느 서버로 요청이 가는지를 결정한다
fetch("http://localhost:8080/api/auth/signup")  // → api-gateway
fetch("http://localhost:8081/api/auth/signup")  // → account-service (직접, 보안 위반)
fetch("http://localhost:8090/api/auth/signup")  // → frontend-service (잘못된 대상)
```

그래서 `API_BASE` 변수가 중요한 것이다. 모든 `fetch()` 호출이 반드시 api-gateway(`8080`)로 향하도록 이 변수 하나가 강제하고 있다.

---

### TCP 연결은 양방향 파이프다

브라우저가 `localhost:8080`으로 TCP 연결을 열면, 이 연결은 **양방향 파이프**다. 파이프의 한쪽 끝은 브라우저가 들고 있고, 다른 쪽 끝은 api-gateway가 들고 있다.

요청은 브라우저 → api-gateway 방향으로 흐르고, 응답은 api-gateway → 브라우저 방향으로 **같은 파이프를 통해** 되돌아온다. api-gateway가 account-service로부터 응답을 받으면, "이 요청이 어디서 왔지?"를 이미 알고 있기 때문에(아까 열린 그 TCP 연결을 통해서 왔으니까) 그 연결의 반대 방향으로 응답을 돌려보낸다. 그 반대 방향은 브라우저다.

frontend-service는 이 파이프 어디에도 포함되어 있지 않다. 처음부터 이 연결에 참여한 적이 없기 때문에, 응답이 frontend-service를 경유할 물리적인 이유가 없다.

```
[브라우저] ←────── 양방향 TCP 파이프 ──────→ [api-gateway :8080]
     요청 →                                        → account-service
     ← 응답                                        ← account-service
...
### 회원가입 전체 흐름 — 단계별 완전 해부

이제 위의 원리들을 바탕으로 "사용자가 회원가입 버튼을 누르는 순간"부터 "화면에 결과가 표시되기까지"를 8단계로 완전히 분해한다.

---

#### 1단계. 사용자가 브라우저 주소창에 `localhost:8090/sign-up`을 입력한다

브라우저가 URL을 보고 `localhost:8090`으로 TCP 연결을 연다. 거기에 frontend-service가 대기하고 있다. 브라우저가 HTTP GET 요청을 보낸다.
...
#### 2단계. frontend-service의 AuthPageController가 요청을 받아 HTML을 렌더링한다

`AuthPageController`가 `application.yml`에서 `app.api-base-url` 값을 읽어 Model에 담고, Thymeleaf가 `sign-up.html` 템플릿에 그 값을 주입해서 완성된 HTML을 만든다.

```java
// AuthPageController.java
@Value("${app.api-base-url}")
private String apiBase;  // "http://localhost:8080" 이 주입됨

@GetMapping("/sign-up")
public String signUpPage(Model model) {
    model.addAttribute("apiBase", apiBase);  // Thymeleaf에 전달
    return "account/sign-up";               // 템플릿 렌더링
}
```
...
# frontend-service application.yml
app:
  api-base-url: http://localhost:8080  # 이 값이 HTML에 박힌다
```

Thymeleaf가 렌더링한 결과물(브라우저에 전달되는 실제 HTML)에는 JavaScript 코드가 포함되어 있고, `API_BASE` 변수에 실제 값이 이미 채워져 있다.

```html
<!-- 브라우저에 전달되는 최종 HTML (Thymeleaf 렌더링 완료 상태) -->
<script>
    // [[${apiBase}]] 가 실제 값으로 치환된 결과
...
frontend-service는 이 HTML을 브라우저에 전달하는 순간 역할이 끝난다. 이후 어떤 일이 벌어지는지 전혀 관여하지 않는다.

---

#### 3단계. 브라우저가 HTML을 받아서 JavaScript를 메모리에 올린다

브라우저가 HTML을 파싱하면서 `<script>` 블록을 만나면, 그 안의 JavaScript 코드를 자신의 V8 엔진(Chrome 기준)에 올린다. 버튼에 클릭 이벤트 리스너를 등록하는 것도 이 시점에 일어난다. 화면에는 회원가입 폼이 표시된 상태다.

frontend-service는 지금 이 상황을 전혀 모른다. 브라우저와의 TCP 연결은 2단계에서 이미 닫혔다.

---

#### 4단계. 사용자가 폼을 작성하고 가입하기 버튼을 클릭한다

버튼 클릭 이벤트가 발생하면, 브라우저가 3단계에서 등록해둔 JavaScript 함수를 실행한다. 이 함수는 서버가 아니라 **사용자의 컴퓨터 안에 있는 브라우저 엔진이 실행**한다.

함수 안에서 `fetch(API_BASE + '/api/auth/signup', ...)`를 만나는 순간, 브라우저는 `API_BASE`에 저장된 값을 읽는다. 그 값은 `"http://localhost:8080"`이다. 브라우저는 `localhost`의 `8080`번 포트로 새로운 TCP 연결을 열고 HTTP 요청을 보낸다.

```javascript
// 브라우저 안에서 실행되는 코드
// API_BASE = "http://localhost:8080" 이므로
// fetch()는 localhost:8080 으로 TCP 연결을 직접 연다
const response = await fetch(API_BASE + '/api/auth/signup', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ nickname, email, password })
...
이 요청이 `localhost:8080`으로 가는 이유는 단 하나, `API_BASE` 변수에 `8080`이 적혀있기 때문이다. frontend-service(`8090`)를 통하는 것이 아니라, 브라우저가 직접 `8080`으로 연결하는 것이다.

```http
<!-- 브라우저가 localhost:8080으로 보내는 실제 HTTP 요청 -->
POST /api/auth/signup HTTP/1.1
Host: localhost:8080
Content-Type: application/json

{"nickname": "양균", "email": "yang@email.com", "password": "pass1234!"}
```

---

#### 5단계. api-gateway가 요청을 받아 라우팅 규칙을 확인한다

api-gateway는 `8080`번 포트에서 대기하고 있다가 요청을 받는다. `application.yml`의 라우팅 규칙을 순서대로 확인한다.
...
spring:
  cloud:
    gateway:
      routes:
        - id: account-service-public
          uri: lb://ACCOUNT-SERVICE   # 목적지: Eureka에서 찾을 서비스 이름
          predicates:
            - Path=/api/auth/**       # /api/auth/ 로 시작하는 모든 경로
          # filters 없음 → JWT 검증 생략, 그냥 통과
```

`/api/auth/signup`이 `/api/auth/**` 패턴에 매칭되므로, 이 요청은 `ACCOUNT-SERVICE`로 보내기로 결정된다. 여기서 `lb://`는 "Eureka에 등록된 서비스 이름으로 실제 주소를 찾아라"는 의미다.

api-gateway는 Eureka 서버(`localhost:8761`)에 "ACCOUNT-SERVICE의 실제 주소가 뭐야?"라고 질의한다.

```
api-gateway → Eureka: "ACCOUNT-SERVICE 어디 있어?"
Eureka → api-gateway: "localhost:8081 에 있어"
api-gateway: 그러면 localhost:8081로 보내야겠다
```

---

#### 6단계. api-gateway가 account-service로 요청을 전달하고 응답을 받는다

api-gateway가 `localhost:8081`로 요청을 그대로 전달한다. account-service가 회원을 저장하고 이메일을 발송한 뒤 JSON 응답을 반환한다.

```java
// account-service AuthController.java
@PostMapping("/api/auth/signup")
public ResponseEntity<?> signUp(@RequestBody SignUpRequest request) {
    // 회원 저장, 이메일 발송 처리
    signUpService.processNewAccount(request);

    // HTML을 렌더링하지 않는다. 순수 JSON 응답만 반환한다.
    return ResponseEntity.status(HttpStatus.CREATED)
            .body(CommonApiResponse.success(null));
}
```

account-service의 응답은 이렇게 생겼다.
...
#### 7단계. api-gateway가 응답을 브라우저로 돌려보낸다

api-gateway는 account-service로부터 받은 JSON 응답을 4단계에서 브라우저가 열어둔 TCP 연결을 통해 그대로 돌려보낸다. 이 응답이 frontend-service를 경유하지 않는 이유는, 4단계에서 브라우저가 연결을 연 상대방이 처음부터 api-gateway(`8080`)였기 때문이다. frontend-service(`8090`)는 이 TCP 연결에 참여한 적이 없다.

```
[브라우저]
    ↕  4단계에서 열린 TCP 연결 (localhost:8080)
[api-gateway :8080]
    ↕  내부 연결 (localhost:8081)
[account-service :8081]

[frontend-service :8090]  ← 이 연결 구조 어디에도 없다
```

---

#### 8단계. 브라우저의 JavaScript가 응답을 받아 DOM을 조작한다

`await fetch(...)` 이후의 코드가 실행된다. 응답이 성공(201)이면 `window.location.href`로 페이지를 이동한다. 이것도 서버가 하는 게 아니라 브라우저 안의 JavaScript가 하는 것이다.
...
if (response.ok) {
    // 성공: 페이지 이동. 이메일을 URL 파라미터로 넘긴다.
    // window.location.href 도 브라우저 기능이다. 서버가 redirect 하는 게 아니다.
    window.location.href = '/check-email?email=' + encodeURIComponent(email);
} else {
    // 실패: 에러 메시지를 DOM에 직접 삽입
    const errorCode = result.errorCode;
    if (errorCode === 'DUPLICATE_NICKNAME') {
        const el = document.getElementById('nicknameError');
        el.textContent = result.message;
...
`window.location.href = '/check-email?email=...'`이 실행되면, 브라우저가 `localhost:8090/check-email?email=...`으로 새로운 GET 요청을 보낸다. 이것이 **1단계와 동일한 흐름의 시작점**이다. frontend-service가 HTML을 내려주고, 그 HTML 안의 JavaScript가 다시 api-gateway로 API를 호출하는 흐름이 반복된다.
...
```
[사용자: 브라우저 주소창에 localhost:8090/sign-up 입력]
         |
         | ① GET localhost:8090/sign-up
         ↓
[frontend-service :8090]
  AuthPageController.signUpPage()
    → application.yml에서 api-base-url 읽음
    → model.addAttribute("apiBase", "http://localhost:8080")
    → Thymeleaf가 sign-up.html 렌더링
    → [[${apiBase}]] 를 "http://localhost:8080" 으로 치환
         |
         | ② 완성된 HTML + JS 코드 반환
         |   (이 시점에 frontend-service의 역할 완전히 종료)
         ↓
[브라우저]
  HTML 파싱 → 화면에 폼 표시
  JS 파싱 → 버튼 클릭 이벤트 리스너 등록
  const API_BASE = "http://localhost:8080" 메모리에 저장
         |
         | [사용자가 폼 작성 후 가입하기 버튼 클릭]
         | 브라우저가 JS 함수 실행
         | fetch() 가 API_BASE(8080)로 새 TCP 연결 생성
         |
         | ③ POST localhost:8080/api/auth/signup
         |    { nickname, email, password } JSON
         |    (frontend-service 전혀 관여 없음)
         ↓
[api-gateway :8080]
  application.yml 라우팅 규칙 확인
    Path=/api/auth/** → ACCOUNT-SERVICE
    filter 없음 → JWT 검증 생략
  Eureka에 ACCOUNT-SERVICE 주소 질의
    → localhost:8081
         |
         | ④ POST localhost:8081/api/auth/signup 전달
         ↓
[account-service :8081]
  SignUpService.processNewAccount()
    → 회원 저장 (DB)
    → 이메일 발송 (인증 링크: localhost:8090/check-email-token?token=...)
  return ResponseEntity.status(201).body(...)  ← JSON 반환, HTML 렌더링 없음
         |
         | ⑤ HTTP 201 Created + JSON 응답 반환
         ↓
[api-gateway :8080]
  account-service의 JSON 응답을
  ③에서 브라우저가 열어둔 TCP 연결로 그대로 전달
         |
         | ⑥ JSON 응답이 브라우저로 도달
         |   (frontend-service 경유 없음 — 처음부터 이 연결에 없었으므로)
         ↓
[브라우저]
  await fetch() 완료 → JS 계속 실행
  response.ok → window.location.href = '/check-email?email=...'
  브라우저가 새 GET 요청 → localhost:8090/check-email?email=...
  → 다시 1단계와 동일한 흐름 시작
```
...
모노리틱에서는 "데이터 처리"와 "HTML 렌더링"이 서버에서 동시에 일어났다. 버튼을 누르면 서버가 처리하고 새 HTML을 만들어서 내려줬다. 브라우저는 그걸 받아서 보여주기만 했다.

MSA에서는 이 두 가지가 완전히 분리됐다. "HTML 렌더링"은 frontend-service가 최초 페이지 요청 때 한 번만 하고 끝낸다. "데이터 처리"는 브라우저의 JavaScript가 api-gateway와 직접 대화해서 수행한다. frontend-service는 두 번째 대화에 끼어들지 않는다.

어느 서버로 요청이 가는지는 `fetch()` 안의 URL, 구체적으로는 포트 번호가 결정한다. `API_BASE = "http://localhost:8080"`이 `8080`을 가리키고 있기 때문에 api-gateway로 가는 것이다. 이것이 MSA 프론트엔드 통신의 전부다.
